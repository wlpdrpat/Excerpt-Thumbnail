<?php
/**
 * Admin-area functionality.
 *
 * Provides a Settings API page under “Settings → Excerpt Thumbnail”
 * and registers/sanitizes plugin options.
 *
 * @link      https://wellplanet.com
 * @since     1.0.0
 * @package   Excerpt_Thumbnail
 * @subpackage Excerpt_Thumbnail/admin
 * @author    Patrick Coleman
 * @license   GPL-2.0-or-later
 */

defined( 'ABSPATH' ) || exit;

/**
 * Enqueues admin assets and registers the settings screen.
 *
 * @since 1.0.0
 */
class Excerpt_Thumbnail_Admin {

	/**
	 * Plugin slug / text domain.
	 *
	 * @since 1.0.0
	 * @var string
	 */
	private $slug;

	/**
	 * Plugin version.
	 *
	 * @since 1.0.0
	 * @var string
	 */
	private $version;

	/**
	 * Settings API group slug.
	 *
	 * @since 1.0.0
	 * @var string
	 */
	private $settings_group = 'excerpt_thumbnail_options';

	/**
	 * Settings page slug (screen id).
	 *
	 * @since 1.0.0
	 * @var string
	 */
	private $page_slug = 'excerpt-thumbnail';

	/**
	 * Constructor.
	 *
	 * @since 1.0.0
	 * @param string $slug    Plugin slug / text domain.
	 * @param string $version Plugin version.
	 */
	public function __construct( $slug, $version ) {
		$this->slug    = $slug;
		$this->version = $version;
	}

	/**
	 * Placeholder for enqueueing admin styles.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function enqueue_styles() {
		// Intentionally empty for now.
	}

	/**
	 * Placeholder for enqueueing admin scripts.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function enqueue_scripts() {
		// Intentionally empty for now.
	}

	/**
	 * Add the options page under Settings.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function add_options_page() {
		add_options_page(
			__( 'Excerpt Thumbnail', 'excerpt-thumbnail' ),
			__( 'Excerpt Thumbnail', 'excerpt-thumbnail' ),
			'manage_options',
			$this->page_slug,
			[ $this, 'render_settings_page' ]
		);
	}

	/**
	 * Register options and settings fields using the Settings API.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function register_settings() {
		if ( ! current_user_can( 'manage_options' ) ) {
			return;
		}

		// --- Register each option with dedicated sanitizers ---
		register_setting( $this->settings_group, 'excerpt_thumbnail_width',               [ $this, 'sanitize_absint' ] );
		register_setting( $this->settings_group, 'excerpt_thumbnail_height',              [ $this, 'sanitize_absint' ] );
		register_setting( $this->settings_group, 'excerpt_thumbnail_align',               [ $this, 'sanitize_align' ] );
		register_setting( $this->settings_group, 'excerpt_thumbnail_default_image',       [ $this, 'sanitize_yesno' ] );
		register_setting( $this->settings_group, 'excerpt_thumbnail_default_image_src',   [ $this, 'sanitize_url' ] );
		register_setting( $this->settings_group, 'excerpt_thumbnail_withlink',            [ $this, 'sanitize_yesno' ] );
		register_setting( $this->settings_group, 'excerpt_thumbnail_on_home',             [ $this, 'sanitize_yesno' ] );
		register_setting( $this->settings_group, 'excerpt_thumbnail_on_archives',         [ $this, 'sanitize_yesno' ] );
		register_setting( $this->settings_group, 'excerpt_thumbnail_on_search',           [ $this, 'sanitize_yesno' ] );
		register_setting( $this->settings_group, 'excerpt_thumbnail_exclusion',           [ $this, 'sanitize_csv_ids' ] );
		register_setting( $this->settings_group, 'excerpt_thumbnail_add_og_image',        [ $this, 'sanitize_yesno' ] );
		register_setting( $this->settings_group, 'excerpt_thumbnail_modern_mode',         [ $this, 'sanitize_yesno' ] );
		register_setting( $this->settings_group, 'excerpt_thumbnail_cleanup_on_uninstall', [ $this, 'sanitize_yesno' ] );

		// --- Section ---
		add_settings_section(
			'excerpt_thumbnail_main',
			__( 'Excerpt Thumbnail Settings', 'excerpt-thumbnail' ),
			function () {
				echo '<p>' . esc_html__( 'Control how thumbnails are added to excerpts on home, archive, and search pages.', 'excerpt-thumbnail' ) . '</p>';
			},
			$this->page_slug
		);

		// --- Fields ---
		add_settings_field(
			'excerpt_thumbnail_width',
			__( 'Image Width (px)', 'excerpt-thumbnail' ),
			[ $this, 'field_width' ],
			$this->page_slug,
			'excerpt_thumbnail_main'
		);

		add_settings_field(
			'excerpt_thumbnail_height',
			__( 'Image Height (px)', 'excerpt-thumbnail' ),
			[ $this, 'field_height' ],
			$this->page_slug,
			'excerpt_thumbnail_main'
		);

		add_settings_field(
			'excerpt_thumbnail_align',
			__( 'Alignment', 'excerpt-thumbnail' ),
			[ $this, 'field_align' ],
			$this->page_slug,
			'excerpt_thumbnail_main'
		);

		add_settings_field(
			'excerpt_thumbnail_withlink',
			__( 'Link Image to Post', 'excerpt-thumbnail' ),
			[ $this, 'field_withlink' ],
			$this->page_slug,
			'excerpt_thumbnail_main'
		);

		add_settings_field(
			'excerpt_thumbnail_default_image',
			__( 'Use Default Image if none found', 'excerpt-thumbnail' ),
			[ $this, 'field_default_image' ],
			$this->page_slug,
			'excerpt_thumbnail_main'
		);

		add_settings_field(
			'excerpt_thumbnail_default_image_src',
			__( 'Default Image URL', 'excerpt-thumbnail' ),
			[ $this, 'field_default_image_src' ],
			$this->page_slug,
			'excerpt_thumbnail_main'
		);

		add_settings_field(
			'excerpt_thumbnail_contexts',
			__( 'Where to show thumbnails', 'excerpt-thumbnail' ),
			[ $this, 'field_contexts' ],
			$this->page_slug,
			'excerpt_thumbnail_main'
		);

		add_settings_field(
			'excerpt_thumbnail_exclusion',
			__( 'Exclude Categories (CSV of IDs)', 'excerpt-thumbnail' ),
			[ $this, 'field_exclusion' ],
			$this->page_slug,
			'excerpt_thumbnail_main'
		);

		add_settings_field(
			'excerpt_thumbnail_add_og_image',
			__( 'Add Open Graph Image', 'excerpt-thumbnail' ),
			[ $this, 'field_add_og_image' ],
			$this->page_slug,
			'excerpt_thumbnail_main'
		);

		add_settings_field(
			'excerpt_thumbnail_modern_mode',
			__( 'Modern Mode (recommended)', 'excerpt-thumbnail' ),
			[ $this, 'field_modern_mode' ],
			$this->page_slug,
			'excerpt_thumbnail_main'
		);

		add_settings_field(
			'excerpt_thumbnail_cleanup_on_uninstall',
			__( 'Remove Data on Uninstall', 'excerpt-thumbnail' ),
			[ $this, 'field_cleanup_on_uninstall' ],
			$this->page_slug,
			'excerpt_thumbnail_main'
		);
	}

	/**
	 * Render the settings page markup.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function render_settings_page() {
		if ( ! current_user_can( 'manage_options' ) ) {
			return;
		}
		?>
		<div class="wrap">
			<h1><?php echo esc_html__( 'Excerpt Thumbnail', 'excerpt-thumbnail' ); ?></h1>
			<form action="options.php" method="post">
				<?php
				settings_fields( $this->settings_group );
				do_settings_sections( $this->page_slug );
				submit_button( __( 'Save Changes', 'excerpt-thumbnail' ) );
				?>
			</form>
			<hr />
			<p>
				<em>
					<?php
					echo esc_html__(
						'Tip: Featured image is used first; if none, the first image in the content is used; if still none and enabled, the default image is used.',
						'excerpt-thumbnail'
					);
					?>
				</em>
			</p>
		</div>
		<?php
	}

	// ===== Helpers ===========================================================

	/**
	 * Read an option with legacy fallback (for safety).
	 * Will check the new key first, otherwise fallback to old tfe_* key if still present.
	 *
	 * @since 1.0.0
	 * @param string     $new_key   New option key (excerpt_thumbnail_*).
	 * @param string     $old_key   Legacy option key (tfe_*).
	 * @param string|int $default   Default if neither present.
	 * @return mixed
	 */
	private function opt( $new_key, $old_key, $default = '' ) {
		$new = get_option( $new_key, null );
		if ( null !== $new && false !== $new ) {
			return $new;
		}
		return get_option( $old_key, $default );
	}

	// ===== Field renderers ===================================================

	/**
	 * Width field.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function field_width() {
		$value = $this->opt( 'excerpt_thumbnail_width', 'tfe_width', 150 );
		printf(
			'<input type="number" min="0" step="1" id="excerpt_thumbnail_width" name="excerpt_thumbnail_width" value="%s" class="small-text" />',
			esc_attr( (string) $value )
		);
	}

	/**
	 * Height field.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function field_height() {
		$value = $this->opt( 'excerpt_thumbnail_height', 'tfe_height', 150 );
		printf(
			'<input type="number" min="0" step="1" id="excerpt_thumbnail_height" name="excerpt_thumbnail_height" value="%s" class="small-text" />',
			esc_attr( (string) $value )
		);
	}

	/**
	 * Alignment field (select).
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function field_align() {
		$value   = $this->opt( 'excerpt_thumbnail_align', 'tfe_align', 'left' );
		$choices = [
			'left'   => __( 'Left', 'excerpt-thumbnail' ),
			'right'  => __( 'Right', 'excerpt-thumbnail' ),
			'center' => __( 'Center', 'excerpt-thumbnail' ),
		];
		echo '<select id="excerpt_thumbnail_align" name="excerpt_thumbnail_align">';
		foreach ( $choices as $key => $label ) {
			printf(
				'<option value="%s"%s>%s</option>',
				esc_attr( $key ),
				selected( $value, $key, false ),
				esc_html( $label )
			);
		}
		echo '</select>';
	}

	/**
	 * Link image to post field (checkbox).
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function field_withlink() {
		$value = $this->opt( 'excerpt_thumbnail_withlink', 'tfe_withlink', 'yes' ); // legacy default: yes
		// Hidden "no" ensures unchecked saves as "no".
		echo '<input type="hidden" name="excerpt_thumbnail_withlink" value="no" />';
		printf(
			'<label><input type="checkbox" name="excerpt_thumbnail_withlink" value="yes"%s> %s</label>',
			checked( $value, 'yes', false ),
			esc_html__( 'Wrap thumbnail in a link to the post', 'excerpt-thumbnail' )
		);
	}

	/**
	 * Use default image field (checkbox).
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function field_default_image() {
		$value = $this->opt( 'excerpt_thumbnail_default_image', 'tfe_default_image', 'no' );
		echo '<input type="hidden" name="excerpt_thumbnail_default_image" value="no" />';
		printf(
			'<label><input type="checkbox" name="excerpt_thumbnail_default_image" value="yes"%s> %s</label>',
			checked( $value, 'yes', false ),
			esc_html__( 'If no image found, use the Default Image URL below', 'excerpt-thumbnail' )
		);
	}

	/**
	 * Default image URL field.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function field_default_image_src() {
		$value = $this->opt( 'excerpt_thumbnail_default_image_src', 'tfe_default_image_src', '' );
		printf(
			'<input type="url" id="excerpt_thumbnail_default_image_src" name="excerpt_thumbnail_default_image_src" value="%s" class="regular-text code" placeholder="https://example.com/path/to/fallback.jpg" />',
			esc_attr( (string) $value )
		);
	}

	/**
	 * Context checkboxes (home, archives, search).
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function field_contexts() {
		$home     = $this->opt( 'excerpt_thumbnail_on_home', 'tfe_on_home', 'yes' );
		$archives = $this->opt( 'excerpt_thumbnail_on_archives', 'tfe_on_archives', 'yes' );
		$search   = $this->opt( 'excerpt_thumbnail_on_search', 'tfe_on_search', 'yes' );

		echo '<fieldset>';

		// Hidden "no" inputs ensure unchecked saves as "no".
		echo '<input type="hidden" name="excerpt_thumbnail_on_home" value="no" />';
		printf(
			'<label><input type="checkbox" name="excerpt_thumbnail_on_home" value="yes"%s> %s</label><br>',
			checked( $home, 'yes', false ),
			esc_html__( 'Home / blog index', 'excerpt-thumbnail' )
		);

		echo '<input type="hidden" name="excerpt_thumbnail_on_archives" value="no" />';
		printf(
			'<label><input type="checkbox" name="excerpt_thumbnail_on_archives" value="yes"%s> %s</label><br>',
			checked( $archives, 'yes', false ),
			esc_html__( 'Category/Tag/Date archives', 'excerpt-thumbnail' )
		);

		echo '<input type="hidden" name="excerpt_thumbnail_on_search" value="no" />';
		printf(
			'<label><input type="checkbox" name="excerpt_thumbnail_on_search" value="yes"%s> %s</label>',
			checked( $search, 'yes', false ),
			esc_html__( 'Search results', 'excerpt-thumbnail' )
		);

		echo '</fieldset>';
	}

	/**
	 * Category exclusions (CSV of IDs).
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function field_exclusion() {
		$value = $this->opt( 'excerpt_thumbnail_exclusion', 'tfe_exclusion', '' );
		printf(
			'<input type="text" id="excerpt_thumbnail_exclusion" name="excerpt_thumbnail_exclusion" value="%s" class="regular-text" placeholder="e.g. 2,7,15" />',
			esc_attr( (string) $value )
		);
		echo '<p class="description">' . esc_html__( 'Enter category IDs to exclude (comma-separated).', 'excerpt-thumbnail' ) . '</p>';
	}

	/**
	 * Add og:image meta tag on single post pages (checkbox).
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function field_add_og_image() {
		$value = $this->opt( 'excerpt_thumbnail_add_og_image', 'tfe_add_og_image', 'yes' ); // default: enabled
		echo '<input type="hidden" name="excerpt_thumbnail_add_og_image" value="no" />';
		printf(
			'<label><input type="checkbox" name="excerpt_thumbnail_add_og_image" value="yes"%s> %s</label>',
			checked( $value, 'yes', false ),
			esc_html__( 'Output <meta property="og:image"> on single posts using the same image logic (Featured → first content image → default).', 'excerpt-thumbnail' )
		);
		echo '<p class="description">' . esc_html__( 'Uncheck if your SEO plugin already sets og:image to avoid duplicates.', 'excerpt-thumbnail' ) . '</p>';
	}

	/**
	 * Modern Mode checkbox.
	 *
	 * When enabled:
	 * - Only modifies the_excerpt (no content forcing).
	 * - Registers a named image size "excerpt-thumbnail".
	 * - Uses that size for featured images.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function field_modern_mode() {
		$value = $this->opt( 'excerpt_thumbnail_modern_mode', 'tfe_modern_mode', 'no' ); // default: legacy behavior
		echo '<input type="hidden" name="excerpt_thumbnail_modern_mode" value="no" />';
		printf(
			'<label><input type="checkbox" name="excerpt_thumbnail_modern_mode" value="yes"%s> %s</label>',
			checked( $value, 'yes', false ),
			esc_html__( 'Use excerpt-only output and the "excerpt-thumbnail" image size. Disable if you rely on legacy content-forcing.', 'excerpt-thumbnail' )
		);
		echo '<p class="description">' . esc_html__( 'Tip: After changing width/height, run a thumbnail regeneration plugin if you want new physical sizes for existing images.', 'excerpt-thumbnail' ) . '</p>';
	}

	/**
	 * Remove settings/data on uninstall (checkbox).
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function field_cleanup_on_uninstall() {
		$value = $this->opt( 'excerpt_thumbnail_cleanup_on_uninstall', 'tfe_cleanup_on_uninstall', 'no' ); // default: keep data
		echo '<input type="hidden" name="excerpt_thumbnail_cleanup_on_uninstall" value="no" />';
		printf(
			'<label><input type="checkbox" name="excerpt_thumbnail_cleanup_on_uninstall" value="yes"%s> %s</label>',
			checked( $value, 'yes', false ),
			esc_html__( 'When the plugin is deleted, remove all Excerpt Thumbnail settings.', 'excerpt-thumbnail' )
		);
		echo '<p class="description">' . esc_html__( 'Recommended on staging/dev. Leave unchecked in production if you may reinstall later.', 'excerpt-thumbnail' ) . '</p>';
	}

	// ===== Sanitizers ========================================================

	/**
	 * Sanitize integer >= 0.
	 *
	 * @since 1.0.0
	 * @param mixed $value Raw value.
	 * @return int
	 */
	public function sanitize_absint( $value ) {
		$v = absint( $value );
		return ( $v < 0 ) ? 0 : $v;
	}

	/**
	 * Sanitize alignment (left|right|center).
	 *
	 * @since 1.0.0
	 * @param mixed $value Raw value.
	 * @return string
	 */
	public function sanitize_align( $value ) {
		$allowed = [ 'left', 'right', 'center' ];
		$value   = is_string( $value ) ? strtolower( $value ) : '';
		return in_array( $value, $allowed, true ) ? $value : 'left';
	}

	/**
	 * Sanitize yes/no toggles.
	 *
	 * For checkboxes: any non-"yes" value becomes "no".
	 *
	 * @since 1.0.0
	 * @param mixed $value Raw value.
	 * @return string "yes"|"no"
	 */
	public function sanitize_yesno( $value ) {
		return ( 'yes' === $value ) ? 'yes' : 'no';
	}

	/**
	 * Sanitize URL (empty allowed).
	 *
	 * @since 1.0.0
	 * @param mixed $value Raw value.
	 * @return string
	 */
	public function sanitize_url( $value ) {
		$value = (string) $value;
		if ( '' === $value ) {
			return '';
		}
		$url = esc_url_raw( $value );
		return ( $url ) ? $url : '';
	}

	/**
	 * Sanitize CSV of category IDs: "1,2,abc,3" -> "1,2,3".
	 *
	 * @since 1.0.0
	 * @param mixed $value Raw value.
	 * @return string Normalized CSV (may be empty).
	 */
	public function sanitize_csv_ids( $value ) {
		if ( ! is_string( $value ) ) {
			return '';
		}
		$parts = preg_split( '/\s*,\s*/', $value, -1, PREG_SPLIT_NO_EMPTY );
		$ids   = [];
		foreach ( (array) $parts as $p ) {
			$n = absint( $p );
			if ( $n > 0 ) {
				$ids[] = (string) $n;
			}
		}
		// Ensure unique and sorted.
		$ids = array_values( array_unique( $ids ) );
		sort( $ids, SORT_NUMERIC );
		return implode( ',', $ids );
	}

	/**
	 * Sanitize plain text (fallback).
	 *
	 * @since 1.0.0
	 * @param mixed $value Raw value.
	 * @return string
	 */
	public function sanitize_text( $value ) {
		return sanitize_text_field( (string) $value );
	}
}
